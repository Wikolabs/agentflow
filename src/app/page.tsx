...
export default function DemoPage() {
  ...
  const t = lang === "fr" ? {
    back: "Retour", title: "Démo", sub: PRODUCT + ", orchestration multi-agents IA",
    desc: "Décrivez un objectif métier complexe (ex: qualifier des leads, automatiser un onboarding). Le compilateur AgentFlow propose les agents, les modèles, la sequence et les garde-fous. Aucune exécution réelle, POC qui illustre la couche d'orchestration.",
    inputLabel: "Objectif métier",
    placeholder: "Ex: traiter une demande de credit client, de la réception du dossier à la décision finale...",
    ...
    mockAirflow: "Exporter vers Airflow", mockDeploy: "Déployer sur AgentFlow",
    sentN8n: "Workflow exporté au format n8n (mode démo, pas d'instance réelle)",
    sentZapier: "Workflow exporté au format Zapier (mode démo, pas d'API réelle)",
    sentAirflow: "DAG Airflow généré (mode démo, pas de cluster réel)",
    sentDeploy: "Workflow déployé sur AgentFlow runtime (mode démo, pas de runtime réel)",
    fallback: "Mode statique : la clé LLM sera ajoutée au prochain déploiement.",
    poweredBy: "Modèle :",
    note: "DEMO POC, pas d'execution réelle d'agents, pas d'export n8n/Zapier/Airflow. L'IA simule le compilateur d'orchestration.",
  } : {
  ...
  async function generate() {
    ...
    setLoading(true);
    try {
      const r = await fetch("/offers/agentflow/demo/api/demo", {
        method: "POST",
    ...
    <div style={{ minHeight: "100vh", background: PAL.bg, color: PAL.txt1, display: "flex", flexDirection: "column" }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .wk-textarea { width: 100%; padding: 12px 14px; border-radius: 6px; background: ${PAL.surface}; border: 1px solid ${PAL.border}; color: ${PAL.txt1}; font-family: inherit; font-size: 14px; resize: vertical; min-height: 130px; transition: border-color .2s, background .2s; line-height: 1.5; }
